...
export function evaluateComponentEligibility(
  ...
  if (component.status !== 'approved') {
    reasons.push(`status is ${component.status}`);
  }

  if (!hasComponentImplementation(component.id)) {
    reasons.push('render implementation missing');
  }

  if (!getContentContract(component.id)) {
    reasons.push('content contract missing');
  }
