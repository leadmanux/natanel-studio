...
export class ShopifyThemeExporter implements SiteExporter {
  ...
  async export(project: Project): Promise<ExportResult> {
    ...
    try {
      ...
      zip.file('sections/ns-header.liquid', buildNativeHeader(project, headerEntry?.section, processedAssets));
...
function buildNativeHeader(
  project: Project,
  sourceSection?: SiteSection,
  processedAssets?: CollectedProjectAssets
): string {
  ...
  const logoAsset = project.assets.find(
    (asset) =>
      asset.outputUrl &&
      (asset.type === 'logo' || asset.referenceCategory === 'logo')
  );
  const logoFilename = logoAsset && processedAssets
    ? processedAssets.idToFilenameMap.get(logoAsset.id) || ''
    : '';
  const logoMarkup = logoFilename
    ? `<img class="ns-header-logo" src="{{ '${escapeLiquidString(logoFilename)}' | asset_url }}" alt="${brand}" loading="eager">`
    : `<span class="ns-header-monogram">${monogram}</span>`;
  return centered
  ...
  <div class="ns-container ns-header-grid">
    ...
    <a class="ns-header-brand" href="{{ routes.root_url }}">${logoMarkup}<span>${brand}</span></a>
